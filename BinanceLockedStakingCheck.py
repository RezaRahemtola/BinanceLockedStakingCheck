...
driver.find_element_by_css_selector(".css-1r19zeh").click()  # Click on the expand staking products button
...
# Catching names of the provided crypto for locked staking
...
for row in driver.find_elements_by_css_selector("div#modal-scroller .css-1pp7m4q"):
    ...
    if numberOfButtons == 1:
        # Only 30 days staking is provided, changing values to False for 60 & 90 days
        available60[i], available90[i] = False, False
    elif numberOfButtons == 2:
        # Only 30 & 60 days staking are provided, changing values to False for 90 days
        available90[i] = False
    i += 1
...
with open(currentPath+"/watchlist.txt", "r") as file:
    for line in file:
        ...
        if result[name][period][0]:
            ...
            toaster.show_toast("Binance Locked Stacking", f"{name} staking for {period} days with a {result[name][period][1]} APY is available !", icon_path=currentPath+"/Binance.ico", duration=None)
